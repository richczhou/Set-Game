...
class SetGame:
    ...
    def showCard(self, cardInt):
        ...
        print(self.cardNums[thouDigit-1], self.cardShading[hunsDigit-1],
              self.cardColor[tensDigit-1], self.cardShapes[unitDigit-1], end="") #end="" means no newline
    ...
    def playSet(self):
        ...
        for i in range(0,3):  #this puts the starting 9 cards. last 3 are in while loop
            self.drawTriple()   #bc it needs to draw on a deadlocked board
        while len(self.table) != 0 and len(self.sets) < 20: #we stop at twenty sets bc we can get a
            self.drawTriple()                                 #20 card or 7 set deadlock
            self.showTable()
            self.isSet()
        lastLen = len(self.sets) #this is so we know when to refresh table
        for i in range(0,10):  #Do the last several ones "manually" so if it is deadlocked
            self.drawTriple()    #it doesn't get stuck forever on an impossible board
            if len(self.sets) > lastLen: #if we found another Set! then we refresh table state
                self.showTable()
